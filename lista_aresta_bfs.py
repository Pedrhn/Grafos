...
def busca_em_largura(vertices, arestas, inicio):
    if inicio not in vertices:
        print("Vértice inicial inexistente.")
        return []
    ...
    while fila:
        ...
        if atual not in visitados:
            ...
            for viz in vizinhos(vertices, arestas, atual):
                if viz not in visitados and viz not in fila:
                    fila.append(viz)

    print("Ordem de visita (BFS):", " -> ".join(visitados))
    return visitados

def menor_caminho_bfs(vertices, arestas, origem, destino):
...
def main():
    ...
    while True:
        ...
        if opcao == "1":
            ...
            busca_em_largura(vertices, arestas, inicio)
        elif opcao == "11":
            o = input("Origem: ")
            d = input("Destino: ")
            menor_caminho_bfs(vertices, arestas, o, d)
        elif opcao == "0":
            print("Encerrando...")
            break
        else:
